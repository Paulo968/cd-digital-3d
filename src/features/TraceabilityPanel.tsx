...
export function TraceabilityPanel({locations,events,query,selectedAddress,onQuery,onSelect}:{locations:WarehouseLocation[];events:TraceEvent[];query:string;selectedAddress:string|null;onQuery:(value:string)=>void;onSelect:(address:string)=>void}){
 const [qrUrl,setQrUrl]=useState('');const [scanFeedback,setScanFeedback]=useState('');const cameraInput=useRef<HTMLInputElement | null>(null)
 ...
 useEffect(()=>{let active=true;if(!qrValue){setQrUrl('');return}QRCode.toDataURL(qrValue,{margin:1,width:240,errorCorrectionLevel:'M'}).then(url=>{if(active)setQrUrl(url)}).catch(()=>{if(active)setQrUrl('')});return()=>{active=false}},[qrValue])
 ...
 return<section className="panel-section">
  ...
  {selected&&<article className="trace-card">
    <div className="trace-identity"><div><span className="eyebrow">Estado atual</span><h3>{selected.handlingUnitCode??selected.address}</h3><p>{selected.description??'Posição sem produto informado'}</p></div>{qrUrl&&<img src={qrUrl} alt={`QR Code de ${selected.handlingUnitCode??selected.address}`}/>}</div>
