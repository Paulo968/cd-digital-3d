...
export function probeDynamicSafety(input: SafetyProbeInput): SafetyProbeResult {
  ...
  for (const hazard of input.hazards) {
    ...
    const predictedClearance = clearanceAtTime(
      { x: deltaX, z: deltaZ },
      relativeVelocity,
      Math.min(predictionHorizon, Number.isFinite(timeToCollision) ? timeToCollision : predictionHorizon),
      combinedRadius,
    )
    const inCurrentLane =
    ...
    const urgency =
      freeDistance +
      (Number.isFinite(timeToCollision) ? timeToCollision * Math.max(0.5, input.speed) : 1000)
